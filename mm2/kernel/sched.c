...
struct timer_struct *task_timer;
...
{
	int i;
	struct task_struct *task, *idle;
	struct segment_descriptor *gdt = (struct segment_descriptor *) ADR_GDT;

	taskctl = (struct task_controller *) memman_alloc_4k(memman, sizeof (struct task_controller));
	...
	idle = task_alloc();
	idle->tss.esp = memman_alloc_4k(memman, 64 * 1024) + 64 * 1024;//stack set 64KB
...
{
	struct task_level *tl = &taskctl->level[taskctl->now_lv];
	struct task_struct *new_task, *now_task = tl->tasks[tl->now];
	tl->now++;
	if (tl->now == tl->running) {
		tl->now = 0;
	}
